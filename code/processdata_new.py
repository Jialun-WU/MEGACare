...
def create_str_token_mapping(df):
    ...
    dill.dump(obj={'diag_voc':diag_voc, 'med_voc':med_voc ,'pro_voc':pro_voc}, file=open('voc_final_new.pkl','wb'))
...
def create_patient_record(df, diag_voc, med_voc, pro_voc):
    ...
    dill.dump(obj=records, file=open('records_final_new.pkl', 'wb'))
    return records

def get_ddi_matrix(records, med_voc, ddi_file_pos, ddi_file_neg, ddi_file_all):
    cid2atc_dic = defaultdict(set)
    med_voc_size = len(med_voc.idx2word) 
    med_unique_word = [med_voc.idx2word[i] for i in range(med_voc_size)]
    atc3_atc4_dic = defaultdict(set)
    for item in med_unique_word:
        atc3_atc4_dic[item].add(item)
    ...
    # ddi load  
    ddi_df_pos = pd.read_csv(ddi_file_pos)
    ddi_df_neg = pd.read_csv(ddi_file_neg)
    ddi_df_all = pd.read_csv(ddi_file_all)
    
    ddi_most_pd_pos = ddi_df_pos.groupby(by=['Y', 'Map']).size().reset_index().rename(columns={0:'count'}).sort_values(by=['count'],ascending=False).reset_index(drop=True)
    ddi_most_pd_neg = ddi_df_neg.groupby(by=['Y', 'Map']).size().reset_index().rename(columns={0:'count'}).sort_values(by=['count'],ascending=False).reset_index(drop=True)
    ddi_most_pd_all = ddi_df_all.groupby(by=['Y', 'Map']).size().reset_index().rename(columns={0:'count'}).sort_values(by=['count'],ascending=False).reset_index(drop=True)
    
    fliter_ddi_df_pos = ddi_df_pos.merge(ddi_most_pd_pos[['Y']], how='inner', on=['Y'])
    fliter_ddi_df_neg = ddi_df_neg.merge(ddi_most_pd_neg[['Y']], how='inner', on=['Y'])
    fliter_ddi_df_all = ddi_df_all.merge(ddi_most_pd_all[['Y']], how='inner', on=['Y'])
    
    ddi_df_pos = fliter_ddi_df_pos[['CID1','CID2']].drop_duplicates().reset_index(drop=True)
    ddi_df_neg = fliter_ddi_df_neg[['CID1','CID2']].drop_duplicates().reset_index(drop=True)
    ddi_df_all = fliter_ddi_df_all[['CID1','CID2']].drop_duplicates().reset_index(drop=True)
    ...
    dill.dump(ehr_adj, open('ehr_adj_final_new.pkl', 'wb'))  


    # ddi adj
    ddi_adj_pos = np.zeros((med_voc_size,med_voc_size))
    for index, row in ddi_df_pos.iterrows():
        # ddi
        cid1 = row['CID1']
        cid2 = row['CID2']
                ...
                for i in atc3_atc4_dic[atc_i]:
                    for j in atc3_atc4_dic[atc_j]:
                        if med_voc.word2idx[i] != med_voc.word2idx[j]:
                            ddi_adj_pos[med_voc.word2idx[i], med_voc.word2idx[j]] = 1
                            ddi_adj_pos[med_voc.word2idx[j], med_voc.word2idx[i]] = 1
    dill.dump(ddi_adj_pos, open('SDDI.pkl', 'wb'))
    
    
    ddi_adj_neg = np.zeros((med_voc_size,med_voc_size))
    
    for index, row in ddi_df_all.iterrows():
        # ddi
        cid1 = row['CID1']
        cid2 = row['CID2']
        ...
        for atc_i in cid2atc_dic[cid1]:
            for atc_j in cid2atc_dic[cid2]:
                ...
                for i in atc3_atc4_dic[atc_i]:
                    for j in atc3_atc4_dic[atc_j]:
                        if med_voc.word2idx[i] != med_voc.word2idx[j]:
                            ddi_adj_neg[med_voc.word2idx[i], med_voc.word2idx[j]] = 1
                            ddi_adj_neg[med_voc.word2idx[j], med_voc.word2idx[i]] = 1
        ...
        for atc_i in cid2atc_dic[cid1]:
            for atc_j in cid2atc_dic[cid2]:
                ...
                for i in atc3_atc4_dic[atc_i]:
                    for j in atc3_atc4_dic[atc_j]:
                        if med_voc.word2idx[i] != med_voc.word2idx[j]:
                            ddi_adj_neg[med_voc.word2idx[i], med_voc.word2idx[j]] = 0
                            ddi_adj_neg[med_voc.word2idx[j], med_voc.word2idx[i]] = 0
    
    for index, row in ddi_df_neg.iterrows():
        # ddi
        cid1 = row['CID1']
        cid2 = row['CID2']
                ...
                for i in atc3_atc4_dic[atc_i]:
                    for j in atc3_atc4_dic[atc_j]:
                        if med_voc.word2idx[i] != med_voc.word2idx[j]:
                            ddi_adj_neg[med_voc.word2idx[i], med_voc.word2idx[j]] = 1
                            ddi_adj_neg[med_voc.word2idx[j], med_voc.word2idx[i]] = 1
    dill.dump(ddi_adj_neg, open('ADDI.pkl', 'wb'))

    return ddi_adj_neg, ddi_adj_pos
...
if __name__ == '__main__':
    ...
    med_structure_file = 'ATC2smiles_new.pkl'
    ...
    # ddi information
    ddi_file_pos = 'DDI_DB_pos.csv'
    ddi_file_neg = 'DDI_DB_neg.csv'
    ddi_file_all = 'DDI_DB_all_v1.csv'
    ...
    statistics(data)
    data.to_csv('data_final_new.csv')
    ...
    ddi_adj = get_ddi_matrix(records, med_voc, ddi_file_pos, ddi_file_neg, ddi_file_all)    